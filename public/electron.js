...
function currentMonth() {
  // these CANNOT return an array bc "getJobs" won't work
  let date = new Date();
  return {
    dateMonth: String(date.getMonth() + 1),
    dateYear: String(date.getFullYear()),
...
ipcMain.on("filter-jobs", (event, filterSpecs) => {
  if (filterSpecs.dateWeek) {
    if (filterSpecs.dateDay) {
      // in case Day or Month are specified...
      delete filterSpecs.dateDay;
    }
    if (filterSpecs.dateMonth) {
      // ...but their input should get blocked on the front end
