...
class DataStorage extends Store {
  ...
  getJobs(filterSpecs) {
    this.filteredJobs = this.jobs.filter((job) =>
      filterSpecs.some((spec) =>
        Object.entries(spec).every(
          ([key, value]) => value === undefined || job[key] === value
        )
      )
    );
    return this;
  ...
  editExpense(editItems) {
    this.expenses = this.expenses.map((expense) => {
      // edits real expenses in storage
      return expense.id === editItems.id
        ? Object.assign({}, expense, editItems)
        : expense;
    });
    this.filteredExpenses = this.filteredExpenses.map((expense) => {
      // edits temporary filtered expenses being displayed
      return expense.id === editItems.id
        ? Object.assign({}, expense, editItems)
        : expense;
    });
  ...
  deleteExpense(id) {
    ...
    ); // deletes expense from temporary filtered expenses being displayed
    return this.saveExpenses();
  }
